...
console.log(config.abbrevsOn); // true
console.log(config.visual.arcSpacing);

// window.BratFrontendEditor = function (element, collData, docData, options) {
/*
function BratFrontendEditorConstructor(element, collData, docData, options) {
...
export class BratFrontendEditor {
    constructor(element, collData, docData, options) {
        ...
        // If option defined, over-write newOptions
        for (var opt in oldOptions) {
            if (oldOptions.hasOwnProperty(opt)) {
                newOptions[opt] = oldOptions[opt];
            }
        }
