...
function loadJson(data) {
    ...
    for (let i = 0; i < parsedJson.data.length; i++) {
        ...
        //let mappedData = { ...parsedJson.data[i], spans: new Map(Object.entries(parsedJson.data[i].spans))}
        outputData.push({ ...parsedJson.data[i], spans: new Map(Object.entries(parsedJson.data[i].spans)) });
        console.log(parsedJson.data[i])
    }
...
function downloadButtonClicked() {
    ...
    let cleanedOutputData = outputData.map(
        (data) => { return { ...data, spans: Object.fromEntries(data.spans) } })
    //let cleanedOutputData = [];

    /*
    for (let i = 0; i < outputData.length; i++) {
        cleanedOutputData[i] = outputData[i].labels
        for (let j = 0; j < outputData[i].label.length; j++) {
            cleanedOutputData[i].label[j] = outputData[i].label[j].substring(getSubLabelLevel(outputData[i].label[j]));
        }
    }*/
