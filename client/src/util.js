...
export const Util = (() => {
    ...
    const objectToUrlStr = function (o) {
        a = [];
    ...
    const profileEnable = function (on) {
        if (on === undefined) on = true;
        profileOn = on;
    }; // profileEnable
    const profileClear = function () {
        if (!profileOn) return;
        profiles = {};
        profileStarts = {};
    }; // profileClear
    const profileStart = function (label) {
        if (!profileOn) return;
        profileStarts[label] = new Date();
    }; // profileStart
    const profileEnd = function (label) {
        if (!profileOn) return;
        const profileElapsed = new Date() - profileStarts[label]
        if (!profiles[label]) profiles[label] = 0;
        profiles[label] += profileElapsed;
    }; // profileEnd
    const profileReport = function () {
        if (!profileOn) return;
        if (window.console) {
    ...
    const embed = function (container, collData, docData, webFontURLs) {
        const dispatcher = new Dispatcher();
        const visualizer = new Visualizer(dispatcher, container, webFontURLs);
    ...
    const embedByURL = function (container, collDataURL, docDataURL, callback) {
        let collData, docData;
        const handler = function () {
            if (collData && docData) {
                const dispatcher = embed(container, collData, docData);
