...
var Visualizer = (function ($, window, undefined) {
    ...
    var Visualizer = function (dispatcher, svgId) {
        ...
        // OPTIONS
        // control how different elements (like annotations, arcs, and highlights) are rendered on the SVG canvas
        ...
        // variables to manage the SVG element, data, collection state, and other aspects of the visualization process
        var svg;
        var $svg;
        var data = null;
        var sourceData = null;
        ...
        // Determines the priority of a comment based on its class.
        ...
        // Clears the SVG content and hides the container.
        ...
        // Marks elements based on specified types, such as sentences, equivalences, spans, or arcs.
        // markedType: A string indicating the type of marking to be applied (e.g., highlight, error, warning).
        var setMarked = function (markedType) {
            // iterates over the items in args[markedType]
            $.each(args[markedType] || [], function (markedNo, marked) {
                // If the item (marked) indicates a sentence (i.e., marked[0] == 'sent'), 
                // it marks the corresponding sentence in data.markedSent as true.
                if (marked[0] == 'sent') {
                    data.markedSent[marked[1]] = true;
                } else if (marked[0] == 'equiv') { // [equiv, Equiv, T1]
                    // If the item is an equivalence (marked[0] == 'equiv'), 
                    // it iterates through sourceData.equivs to find the equivalence that matches marked[1] and marked[2]
                    $.each(sourceData.equivs, function (equivNo, equiv) {
                        if (equiv[1] == marked[1]) {
                            ...
                            for (var i = 2; i < len; i++) {
                                if (equiv[i] == marked[2]) {
                                    // found it
                                    // Once found, it marks the corresponding arcs in data.eventDescs as markedType
                                    len -= 3;
                                    for (var i = 1; i <= len; i++) {
                    ...
                    // If marked has exactly two elements, it is assumed to be a text span
                    // adds it to the markedText array with its coordinates parsed as integers and associated with markedType
                } else if (marked.length == 2) {
                    markedText.push([parseInt(marked[0], 10), parseInt(marked[1], 10), markedType]);
                    // If marked has a different structure:
                } else {
                    // attempts to find the span in data.spans using marked[0].
                    var span = data.spans[marked[0]];
                    // If found, and if marked has three elements, it marks the corresponding outgoing arcs that match marked[1] (type) and marked[2] (target) as markedType
                    if (span) {
                        if (marked.length == 3) { // arc
                            ...
                            // If marked has more than three elements, it marks the span itself as markedType
                        } else { // span
                            span.marked = markedType;
                        }
                        // If no span is found but an event description (eventDesc) is found
                    } else {
                        var eventDesc = data.eventDescs[marked[0]];
                        if (eventDesc) { // relation
                            // Marks the relevant arcs in data.spans as markedType by matching the target and type.
                            ...
                            // If no span or event description is found, it checks all event descriptions to see if any have a trigger matching marked[0]
                        } else { // try for trigger
                            $.each(data.eventDescs, function (eventDescNo, eventDesc) {
                                if (eventDesc.triggerId == marked[0]) {
                                    // If found, it marks the corresponding span as markedType
        ...
        var setData = function (_sourceData) {
            ...
            // ensure all modifications refer to valid events in the document
